...
summary: "Seed e modalità della run; tema e personaggio si scelgono nel Piano 0. Nessun selettore di difficoltà: la curva dei 5 piani è unica e uguale per tutti (DEC-038). Le run condivise tramite codice breve o file RunBundle sono sempre non classificate (DEC-066)."
---
...
| Codice breve di run condivisa (DEC-066) | Sempre | Sempre | Incolla un codice breve (seed più versione di gioco) ricevuto da un altro giocatore | Rigenera localmente una run identica a quella condivisa | Validazione del codice; conferma se la versione di gioco combacia |
...
1. **Codice breve testuale** (seed più versione di gioco): il giocatore lo incolla nel campo
   dedicato; il gioco rigenera localmente i contenuti dallo stesso seed. Richiede che la
   versione di gioco combaci per garantire la stessa generazione.
...
6. **Given** il giocatore incolla un codice breve di run condivisa valido con la stessa versione di gioco, **when** conferma "Avvia", **then** ottiene una run identica a quella condivisa, etichettata come non classificata (DEC-066).
