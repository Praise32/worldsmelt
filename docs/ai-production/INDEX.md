...
- [Pipeline UI e GUI](15-UI-DESIGN-PIPELINE.md) — Proposta di pipeline UI: Penpot come sorgente design, token a 3 livelli, componenti minimi, 9-slice, moduli src/ui/, raygui solo per tool interni. `[proposed/supporting]`
- [Pipeline audio](16-AUDIO-GENERATION-PIPELINE.md) — Pipeline ibrida rFXGen + Stable Audio 3 Small per SFX/musica, in esplicito conflitto con DEC-036 che considera l'audio generativo futuro. `[proposed/supporting]`
- [Curation degli asset e Piano 0](17-ASSET-CURATION-AND-FLOOR-ZERO.md) — Pipeline di curation (candidate->curated->fallback) con stati, directory art/, manifest e proposta di Piano 0 ibrido curato+generato. `[proposed/supporting]`
- [Orchestrazione degli agenti](18-AGENT-ORCHESTRATION.md) — Path Orchestrator che classifica i task in 7 percorsi (design/technical/ML/UI/audio/curation/implementation) e regola quando porre domande bloccanti. `[proposed/supporting]`
...
- [Licenze dello stack](licenze.md) — Analisi non legale delle licenze di codice (raylib, llama.cpp, stable-diffusion.cpp, Lua, cJSON) e modelli (Qwen, pixel model OpenRAIL-M, LCM-LoRA, TAESD). `[approved/canonical]`
