...
**Batch decisionale del 2026-07-31, seconda sessione — reference distillation (DEC-205...
DEC-210).** Il proprietario ha risposto in sessione remota, con prove visive alla mano, a
un batch di sei decisioni di stile emerse dalla costruzione di `assets/art-library/` (9
pack CC0, 22 reference card, Fase A) e dalle prove dirette in
`assets/art-src/experiments/`: DEC-205 (stile a masse senza outline, **supera** il punto
(a) di DEC-176), DEC-206 (personaggi senza volto), DEC-207 (personaggi senza armi in mano,
precisa DEC-049), DEC-208 (scala 32px **riconfermata**, DEC-177 invariata, 64px esplorato e
scartato), DEC-209 (il processo di reference distillation diventa canone di produzione
art), DEC-210 (piano a onde del rifacimento grafica demo, onda 1 = personaggi+nemici).
Nessuna di queste sei tocca una voce numerata nuova; **la 12** (già chiusa da DEC-176/177)
riceve una nota di aggiornamento perché DEC-205 supera la parte stile della sua chiusura
storica, mentre la parte scala resta riconfermata da DEC-208; **la 18** riceve una nota di
aggiornamento parziale da DEC-210 senza chiudersi.
...
18. Quali body plan si realizzano per primi e con quale approccio si anima il personaggio giocante (rig modulare con skin, spritesheet completo, ibrido, stickman fino alla vertical slice)? (`docs/ai-production/09-NEMICI-BODY-PLAN-RIG.md` propone senza decidere. Provenienza: `Q-ANIM-001` e `Q-ANIM-002`.) **Aggiornamento 31/07 (DEC-210) — non chiude la voce:** «quali per primi» ha ora una risposta di ordine di produzione, non di body plan singoli: **onda 1 = i 3 personaggi della rosa base** (walk 4 direzioni, idle, hit, death) **+ il bestiario organizzato per famiglie di body plan** (tier come recolor di famiglia). Resta aperto «con quale approccio si anima»: il working doc `assets/art-library/30_visual_language/visual-language-v2-DRAFT.md` (sezione «Struttura dei file (rig)») descrive un rig sorgente a livelli per parte (ibrido: livelli separati in Aseprite, esportati come spritesheet completo), ma non è ancora una decisione di design promossa qui — resta principio operativo della distillazione (DEC-209), non canone.
