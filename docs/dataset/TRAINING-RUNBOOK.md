...
"Costi e tempi GPU cloud" e punto 7 della roadmap), `docs/dataset/README.md`.
...
- **Registro di provenienza** aggiornato: `docs/dataset/ledger.jsonl`,
  verificato con `python3 scripts/dataset_ledger.py check` (nessun problema).
- **Baseline congelata**: `docs/dataset/baseline-prompts.txt` (15 coppie
...
Il registro (`docs/dataset/ledger.jsonl`) ha oggi 3158 file da 6 pack, tutti
...
2. **Rigenera gli STESSI prompt/seed della baseline**: usa
   `docs/dataset/baseline-prompts.txt` (le 15 coppie tema|stile, invariate)
...
- [ ] checksum del dataset (il registro con gli sha256 di
      `docs/dataset/ledger.jsonl` copre già questo per i file grezzi);
