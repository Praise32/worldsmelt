...
| `4ad29ab` | **M6a — La rosa dei personaggi** (DEC-030/033). Tre personaggi curati nel pannello TAB del Piano 0 (sezione PERSONAGGI): Wayfinder (esploratore, il default: feel storico +fortuna), Ashblade (vetro: danno 10, tetto vita 8), Bulwark (roccia: lento, tetto vita 16). Il tetto di salute e' PER personaggio come vuole DEC-033. Stickman tinto con la palette. Numeri = default proposti, da playtest (open question 7 aperta). Screenshot: `logs/worldsmelt-floorzero-characters-screen.png`. |
| `a148cf3` | **M6b-1 — Il personaggio forgiato dalla run** (DEC-014, prima fetta). L'IA propone un quarto personaggio (nome, blurb, stats in bande, palette) nella stessa sessione di `--propose-themes` (un solo caricamento modello); carta "FORGED THIS RUN" nel pannello, fallback canonico = carta assente in silenzio. Bande in `src/core/character_type.h`, doppia rete di clamp. Campioni veri: "Elegant Sentinel", "Savage Skewer". Trait Lua e colpo firmato = fette successive (gap annotati in KB). |
| `ecfbba1` | **M6b-2 — Il trait Lua del personaggio** (DEC-037). Il modello scrive il comportamento del personaggio in Lua nella stessa fase propose, dry-run nella sandbox VERA (esattamente una callback), trait invalido = carta assente. In gioco: sandbox whole-run dedicata dietro la facciata ScriptItems, ricalcolo base→trait→oggetti→clamp sempre da zero. Allowlist sandbox INTATTA. Campioni: "Elegant Sentinel" (cadenza ×0.95), "Penetrating Prospector" (danno +2). |
| `688320b` | **Garanzia del motore: il piano 1 è sempre il mondo scelto alla lettera.** Il 1.5B a volte scriveva un nome diverso ("Glass Cathedral" invece di "Foundry of Glass") e il check M5 era flaky: ora la normalizzazione lo forza (come ShotTypeBalance: il modello propone, il motore garantisce), la rete anti-fotocopia risolve le collisioni, test-llm deterministico su quel check. |
| `3e15739` | **M6b-3 — Il colpo firmato** (DEC-068). Il personaggio generato PUÒ avere un colpo proprio ("circa metà delle volte"), sempre un sidegrade in banda (ShotTypeBalance riusato puro); chi ce l'ha paga in stats più caute (frazione 0.6, default proposto — open questions 18/19 restano tue). È il colpo di partenza: gli oggetti-colpo lo sostituiscono come sempre. Campione vero: "Shadow Stealth" per "Elegant Shadows". **Con questo il personaggio generato di DEC-014 è completo: stats + trait Lua + colpo firmato.** |
| `3dbb08c` | **M7 — Il catalogo persistente v1: la memoria del crogiolo** (DEC-015/045/069). A fine run (vittoria/sconfitta/abbandono/reroll) il gioco scrive in `catalog/` un record AUTOSUFFICIENTE dei contenuti generati incontrati (parametri completi + Lua: le Reliquie di DEC-069 si rivalideranno dal catalogo da solo). RunResults dice quante creazioni sono entrate. È il substrato di museo, arene best-of e schermata Catalogo. Qui la scala ha dato la sua PRIMA BOCCIA vera: il verifier ha beccato il test che inquinava `catalog/` (6 file a giro), opus ha corretto con pulizia snapshot-based, giudizio finale mio con prove dirette. |
| `f8055bd` | **M8 — La schermata Catalogo** (DEC-045). La voce "Catalogo" è finalmente nel menu principale: enciclopedia delle creazioni incontrate su 7 categorie con dettaglio per voce, dentro lo stato MainMenu (la mappa canonica dei 9 stati resta intatta — domanda aperta 22 registrata). Altro bug vero stanato: il parser matchava `character.name=` dentro `shot.character.name=` (chiavi ora ancorate a inizio riga). Screenshot: `logs/worldsmelt-catalog-screen.png`. Gap espliciti: preferiti, spesa punti, Reliquie, sprite nelle schede. |
| `7a64417` | **Secondo giro di design 19/07 — le domande residue dei documenti** (DEC-089..108, 20 decisioni). Sancito: abbandono run → RunResults, reroll diretto silenzioso (089); chiusura gioco = dialogo modale (090); tema/personaggio modificabili fino all'uscita, cambio tema riavvia la generazione — gap: M5 tratta la conferma come definitiva (091); arene = simulazioni pure, stato ripristinato, niente economia propria, 1 best-of con semina curata, prove museo illimitate (092-095); seed diversi equi by-design con budget vincolato (096); rifiuto ripensabile, trait anti-fotocopia, colpo firmato sostituibile, sblocchi Ashblade=prima run e Bulwark=primo boss (097-100); fusione libera tra categorie + ri-fusione (101-102); curati nel Catalogo con origine (103); roster nemici estendibile da best-of nei piani avanzati, default piano 3/4 (104); **tono ironico-leggero: il crogiolo ha coscienza di sé** (105); boss piano 2 monofase (106); piega-regole solo leggendari (107); in gara proposte generate identiche ma rosa personale (108). Il verify ha beccato 3 contraddizioni vere (corrette). Restano 11 open questions, quasi tutte numeri da playtest. |
| `dedc317` | **Sessione di design 19/07 — hai risposto alle domande di M5-M8** (DEC-074..088, 15 decisioni). Sancito: ESC→ExitConfirm dal Piano 0 (074); Piano 0 = menu, carte cliccabili col mouse — gap: vanno rese cliccabili (075); 3 carte tema fallback (076); il codice run porta anche tema+personaggio, preselezionati — gap: oggi trasporta solo seed+versione (077); sconto colpo firmato = compressione fissa, lo 0.6 resta da playtest (078); il colpo firmato non si scarta mai, si normalizza (079); Wayfinder/Ashblade/Bulwark canonici nei nomi/ruoli (080); Daily alle 00:00 UTC (081); abbandono/reroll da run in corso = sconfitta per i punti, l'abbandono del Piano 0 no (082+perimetro); 7 categorie del Catalogo approvate (083); Catalogo resta vista interna di MainMenu (084); Reliquie nel museo solo se preferite (085); primo avvio = schermata a due carte, riattivazione in Impostazioni (086); pool curato minimo per categoria, numeri da rivedere (087); minimo gioco base = fase 1 + economia punti DEC-027 + spesa nel Catalogo (088). open-questions rinumerato: restano 11, quasi tutte da playtest; nuova #11 (flusso abbandono: RunResults vs MainMenu, decidila tu). |
...
- **Domanda aperta #14 in KB:** RISOLTA il 19/07 — hai sancito ESC → ExitConfirm (DEC-074),
  l'implementazione era gia' conforme. Le altre risposte del 19/07 sono nella riga `dedc317`
